...
public class SuggestionEngine extends Java8BaseListener {
	...
	public void enterMethodDeclaration(
		...
		Java8Parser.MethodDeclaratorContext methodDeclarator = ctx.methodHeader().methodDeclarator();

		for (Java8Parser.MethodModifierContext modifier : methodModifiers) {
			TerminalNode identifier = methodDeclarator.Identifier();

			if (modifier.getText().contains("public") && !mMethods.contains(identifier.getText())) {
				mMethods.add(identifier.getText());
			}
		}
	}
	...
	private TreeSet<Candidate> getTopKNeighbor(String word, int K) {
		...
		for (String methodName : mMethods) {
			double distance = Levenshtein.distance(word, methodName);
			Candidate method = new Candidate(methodName, distance);
			minHeap.add(method);

			LOGGER.info(method.methodName + ": " + method.distance + "\n");
